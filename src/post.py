...
def index_catalog(
  ...
  url = f"https://{hostname}/{dc_endpoint}/{account_endpoint}/{catalog_endpoint}/indexes"
  ...
  logger.info("Feed API: HTTP POST: %s", response.url)
...
def br_check_status(job_id="", environment_name="", token=""):
  ...
  if state in ["failed", "killed"]:
    ...
    raise ValueError("POST job did not complete successfully")
...
if __name__ == '__main__':
  ...
  parser = argparse.ArgumentParser(
    description="Post/Index the catalog after feed is ingested"
  )

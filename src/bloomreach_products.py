...
def create_products(fp, shopify_url):
    """Function to invoke BR discovery products on each record"""
    products = []
...
def create_product(product, shopify_url):
    """Function creating BR discovery product"""
    out_product = {
      "id": product["id"],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
      ...
      "This is more or less a place holder script to add any transformations necessary"
      "that need to be made on top of the generic product transforms."
      "For instance, if shopify product tags are used in a special way,"
      "custom transforms can be created."
      "Also, generic transforms can be overriden should it be necessary"
      "for a catalog specific behavior."
      "The values of the shopify prefixed attributes should not be modified."
    )
